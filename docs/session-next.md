...
| Plugin | 122 | api_client 32, flush 22, formatter 15, context_writer 8, diff_collector 6, conflict_detector 19, hooks 11 |
...
### 0. ✅ Branch Metadata 0개 버그 수정 (완료)
플러그인이 scaffold repo에서 branch를 감지하지 못하던 이슈 해결:
- **Bug 1**: `get_current_branch()` 등 git 명령이 `cwd` 미전달 → hook CWD가 git repo 아닐 때 실패
- **Bug 2**: `on_session_start.py`에서 pull 결과 0일 때 `save_state()` 전 early return → branch 정보 state 미저장
- **Bug 3**: `store.py` 스키마의 `idx_events_repo_branch` 인덱스가 migration 전에 실행 → 기존 DB 서버 시작 실패
- 수정 후 AB 테스트: Pioneer 9 events → **9/9 branch metadata 정상 전달** (기존 0/9)
- +5 새 테스트 (실제 git repo 기반 CWD branch 감지), flaky test 1개 수정
...
### 3. N=3 M=3 재현성 검증 + Model-Tier 재실행
- branch metadata 수정 후 첫 N=3 M=3 결과 (default model): Student/Naive 차이 미미
- **haiku model-tier 재실행 필요**: `--agent-model haiku --pioneer-model sonnet` 으로 차이 극대화
- Pioneer=opus 테스트도 시도 가능

### 4. Dashboard branch 시각화
...
| Fix | `api_client.py` | git 명령에 명시적 cwd 전달 (branch metadata 0개 수정) |
| Fix | `on_session_start.py` | branch info 즉시 save_state + cwd 전달 |
| Fix | `store.py` | branch index를 migration으로 이동 (기존 DB 호환) |
| Test | `test_api_client.py` | +5 실제 git repo 기반 CWD branch 감지 테스트 |
